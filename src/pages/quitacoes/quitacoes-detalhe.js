...
// ── Seções do comprovante (mesmo markup, uma função por bloco) ─────────────
function _secaoPessoaisHTML(c) {
  return `
...
function _secaoProfissionaisHTML(p) {
  if (!(p.cargo || p.unidade)) return '';
  return `
            <div class="q-comp-section">
...
function _secaoContratoHTML(q) {
  if (!(q.banco || q.contrato)) return '';
  return `
            <div class="q-comp-section">
...
function _secaoPagamentosHTML(c, q, hasQuit) {
  if (!hasQuit) return '';
  return `
            <div class="q-comp-section">
...
function _secaoPagadorHTML(q) {
  if (!q.pag_nome) return '';
  return `
            <div class="q-comp-section">
...
// Barra "anexar documento" quando o cliente ainda não tem doc
function _anexarBarHTML(c, hasDoc) {
  if (hasDoc) return '';
  return `
...
function _painelDocHTML(hasDoc) {
  return hasDoc
      ...
      <div class="q-split-left">
        <div class="q-comprovante">
          ...
          <div class="q-comp-body">
            ...
            ${_secaoPessoaisHTML(c)}
            ${_secaoProfissionaisHTML(p)}
            ${_secaoContratoHTML(q)}
            ${_secaoPagamentosHTML(c, q, hasQuit)}
            ${_secaoPagadorHTML(q)}

          </div><!-- /comp-body -->

          <!-- Anexar documento (se não tiver) -->
          ${_anexarBarHTML(c, hasDoc)}
...
async function _renderDoc(c) {
  ...
  // ── Legado base64 ────────────────────────────────
  if (c.doc_pdf) _renderDocLegado(panel, c.doc_pdf);
}

// PDF/imagem antigo salvo em base64 na coluna doc_pdf (pré-Storage)
function _renderDocLegado(panel, docPdf) {
  if (docPdf.startsWith('data:image/')) {
    panel.innerHTML = `<img src="${docPdf}" style="width:100%;height:auto;border-radius:6px;display:block;box-shadow:0 2px 12px rgba(0,0,0,0.4)">`;
    return;
  }
  try {
    const base64 = docPdf.split(',')[1];
...
function _destinoNuHTML(q) {
  return `<div class="q-nu-section">
