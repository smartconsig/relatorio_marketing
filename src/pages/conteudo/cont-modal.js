...
// Preenche os campos com os valores do card (tabela id → valor).
function _preencherCamposCard(card, respSel) {
  [
    ['cont-f-titulo', card?.titulo],
    ['cont-f-canal',  card?.canal],
    ['cont-f-tipo',   card?.tipo],
    ['cont-f-data',   card?.data_alvo],
    ['cont-f-link',   card?.link_url],
    ['cont-f-desc',   card?.descricao],
...
export function abrirModal(id) {
  ...
  _preencherCamposCard(card, respSel);
  _aplicarPermissoesModal(card, podeEditar);
...
function _statusValidoParaTipo(tipo) {
  const producaoStatus = document.getElementById('cont-f-status').value || null;
  if (!tipo) return null;
  return statusDoTipo(tipo).some(s => s.key === producaoStatus) ? producaoStatus : 'roteiro';
}

function _coletarPayloadCard(titulo, coluna) {
  const tipo = document.getElementById('cont-f-tipo').value || null;
  return {
    titulo,
    canal:          document.getElementById('cont-f-canal').value || null,
    tipo,
    producao_status: _statusValidoParaTipo(tipo),
...
async function _atualizarCardExistente(payload, coluna) {
...
async function _criarCardNovo(payload, coluna) {
...
export async function salvarCard() {
  ...
  const payload = _coletarPayloadCard(titulo, coluna);

  try {
    if (C.editId) await _atualizarCardExistente(payload, coluna);
    else          await _criarCardNovo(payload, coluna);
    fecharModal();
    renderBoard();
  } catch (err) {
