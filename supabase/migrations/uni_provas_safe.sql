-- ── Universidade Smart — Provas, Questões & Certificados (safe) ──────────
-- Versão idempotente: pode rodar quantas vezes quiser sem erro
...
-- Remove policies antigas antes de criar (evita erro de duplicata)
...
create policy "uni_tentativas_user"
  on uni_tentativas for select to authenticated
  using (auth.uid() = user_id or (select is_admin_user()));

create policy "uni_tentativas_insert"
  on uni_tentativas for insert to authenticated
  with check (auth.uid() = user_id);

create policy "uni_certificados_user"
  on uni_certificados for select to authenticated
  using (auth.uid() = user_id or (select is_admin_user()));

create policy "uni_certificados_insert"
  on uni_certificados for insert to authenticated
  with check (auth.uid() = user_id or (select is_admin_user()));
