...
| Etapa B1 | Virada: fichas viram a fonte de leitura; snapshot vira reserva (fallback se contagem não bater) e continua sendo gravado | ✅ produção 07/09/2026 — conferência aprovada (40 comparações, todas OK; 7.479 fichas = contagem esperada; 1 import_id). Primeiro deploy quebrou o salvamento de classificação; corrigido no mesmo dia (`33ceecc4`, trava de carregamento atrasado) e **validado pelo responsável em produção** |
| Etapa C | Remove escrita/leitura do snapshot; clique salva só a linha dele; adicionar **revalidação automática a cada 30s** das `classifications` (pedido do responsável, padrão da Esteira) | após ~1 semana de B1 limpa — **conferir por volta de 14/09/2026** |
...
**Desenho da Etapa B1** (`auth.js` + `propostas-store.js`): o login tem três caminhos, nesta ordem —
1. **Cache em dia**: `checkImportMeta()` (consulta leve) devolve `import_id|updated_at`; se bater com o carimbo salvo em `localStorage` (`sc_fichas_v1`), reusa o cache local e **não relê as milhares de fichas** — só atualiza as decisões (dicionários + `classifications`). É o caminho normal do dia a dia.
2. **Fichas**: carimbo diferente (houve import novo) → `loadImportData()` monta o estado pelas tabelas e regrava o carimbo.
3. **Snapshot (paraquedas)**: só se 1 e 2 falharem. Registra no action-log com tipo `fase3_sombra` ("fichas indisponíveis") — é o alarme a vigiar na semana de B1.

⚠️ **A ficha congela a entrada no momento do import**; decisões humanas tomadas depois vivem nas tabelas próprias. Por isso `resetManualMarks()` desfaz a marca `'manual'` que veio da ficha (devolvendo `isMarketing` à regra do Ecorban) e `syncClassificationsFromSupabase()` reaplica o que ainda vale — é isso que faz um "desfazer classificação" feito em outro computador chegar aos demais. **Isso só acontece no caminho 2 (fichas)**, nunca sobre o cache do caminho 1: apagar a marca manual do cache antes de o `upsert` chegar ao banco fazia a confirmação sumir da tela (bug em produção em 07/09/2026).

⚠️ **Trava contra carregamento atrasado** (`markLocalEdit`/`hasLocalEdits` em `classifications.js`): a tela sobe na hora com o cache, mas a leitura das fichas leva de 2 a 12 segundos. **Toda decisão do usuário marca a sessão** (classificar, desfazer, confirmar divergência, mapear vendedor) e, se houve decisão durante a carga, o login **não aplica** as fichas nem o snapshot — senão o clique daqueles segundos é sobrescrito. Vale também para o download do snapshot, e é causa provável de episódios históricos de "classificação que some sozinha". Ao mexer no login ou criar um caminho novo de carga, **manter essa trava**.

⚠️ `loadImportData()` **rejeita a leitura** (devolve `null` → cai no snapshot) quando `import_meta.propostas_count`/`smart_leads_count` não bate com o que foi lido — defesa contra import interrompido ou concorrente.

⚠️ A marca `reviewReason = 'reclassified'` (desfazer UMA proposta de um CPF que tem outras confirmadas, em `clientes.js`) **não tem tabela própria** e já não sobrevivia a um import antes da B1 (o `buildResult` reaplica overrides por CPF). Com a B1 ela passa a durar até o próximo login. Torná-la durável exige tabela por proposta — candidata a melhoria na Etapa C, não foi feita.
