...
⚠️ **A ficha congela a entrada no momento do import**; decisões humanas tomadas depois vivem nas tabelas próprias. Por isso, nos caminhos 1 e 2, `resetManualMarks()` **desfaz a marca `'manual'` que veio da ficha/cache** (devolvendo `isMarketing` à regra do Ecorban) e logo em seguida `syncClassificationsFromSupabase()` reaplica o que ainda vale. É isso que faz um "desfazer classificação" feito em outro computador chegar aos demais. **Consequência**: uma classificação cujo `upsert` em `classifications` falhou (toast de erro) não sobrevive ao próximo login — é o preço de a tabela ser a fonte da verdade, e está alinhado com a regra do bug recorrente.
